...
const AdmissionPublic = () => {
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `${API_BASE}/api/admission/public/register-student`,
        form
      );
      if (res.data.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:5002/api/admission/public/register-student",
        form
      );
      if (res.data.success) {
        toast.success(res.data.message || "Student registered successfully!");
          ...
          <div className="bg-white rounded-lg shadow-lg p-8 max-w-sm w-full text-center">
            ...
            <div className="mb-2">
              ...
              <span className="font-semibold">
                {modalData.applicationNumber}
              </span>
            </div>
            <div className="mb-2">
              Class: <span className="font-semibold">{modalData.class_}</span>
            </div>
            <div className="mb-4">
              Section:{" "}
              <span className="font-semibold">{modalData.assignedSection}</span>
            </div>
            <a href="/" className="inline-block w-full">
              <button
                type="button"
                className="w-full bg-black text-white py-2 rounded font-semibold mt-2 hover:bg-blue-700 transition"
              >
                Go to Home
              </button>
            </a>
